...
describe('BitChute platform tests', () => {
  ...
  describe('builders', () => {
    test('profile builder', () => {
      expect(mod.buildProfileUrl?.('AnotherChan')).toBe(
        'https://www.bitchute.com/channel/AnotherChan',
      );
    });
    test('video builder', () => {
      expect(mod.buildContentUrl?.('video', 'XYZ123')).toBe(
        'https://www.bitchute.com/video/XYZ123',
      );
    });
  });
});
