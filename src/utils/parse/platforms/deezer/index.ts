...
export const deezer: PlatformModule = {
    ...
    patterns: {
        profile: new RegExp(`^https?://${DOMAIN_PATTERN}/(?:${LOCALE}/)?artist/(\\d+)/?${QUERY_HASH}$`, 'i'),
        handle: /^\d+$/, // artist id numeric
        content: {
            album: new RegExp(`^https?://${DOMAIN_PATTERN}/(?:${LOCALE}/)?album/(\\d+)/?${QUERY_HASH}$`, 'i'),
            track: new RegExp(`^https?://${DOMAIN_PATTERN}/(?:${LOCALE}/)?track/(\\d+)/?${QUERY_HASH}$`, 'i'),
            playlist: new RegExp(`^https?://${DOMAIN_PATTERN}/(?:${LOCALE}/)?playlist/(\\d+)/?${QUERY_HASH}$`, 'i'),
        },
    },
