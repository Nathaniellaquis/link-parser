...
export const artstation: PlatformModule = {
    ...
    patterns: {
        ...
        content: {
            artwork: new RegExp(`^https?://${DOMAIN_PATTERN}/artwork/([A-Za-z0-9]{5,})/?${QUERY_HASH}$`, 'i'),
            project: new RegExp(`^https?://${DOMAIN_PATTERN}/projects/([A-Za-z0-9]{5,})/?${QUERY_HASH}$`, 'i'),
        },
    },
    ...
    extract(url: string, result: ParsedUrl): void {
        const aw = this.patterns.content?.artwork?.exec(url) || this.patterns.content?.project?.exec(url)
        if (aw) {
