...
export const beatport: PlatformModule = {
    ...
    patterns: {
        profile: new RegExp(`^https?://${DOMAIN_PATTERN}/artist/(${slug})/(\\d{1,7})/?${QUERY_HASH}$`, 'i'),
        handle: /^[A-Za-z0-9-]{3,50}$/,
        content: {
            track: new RegExp(`^https?://${DOMAIN_PATTERN}/track/(${slug})/(\\d{1,7})/?${QUERY_HASH}$`, 'i'),
            release: new RegExp(`^https?://${DOMAIN_PATTERN}/release/(${slug})/(\\d{1,7})/?${QUERY_HASH}$`, 'i'),
        },
    },
