...
export const vimeo: PlatformModule = {
    ...
    patterns: {
        profile: new RegExp(`^https?://${DOMAIN_PATTERN}/(?:user\\d+|[A-Za-z0-9_]{3,32})/?${QUERY_HASH}$`, 'i'),
        handle: /^(?:user\d+|[A-Za-z0-9_]{3,32})$/,
        content: {
            video: new RegExp(`^https?://${DOMAIN_PATTERN}/(\\d{6,12})/?${QUERY_HASH}$`, 'i'),
            channel: new RegExp(`^https?://${DOMAIN_PATTERN}/channels/([A-Za-z0-9_-]{3,32})/?${QUERY_HASH}$`, 'i'),
        },
    },
